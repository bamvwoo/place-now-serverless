...
const getCode = async ({ email, verificationCodeValue }) => {
    await connectToDatabase();

    const verificationCode = await VerificationCode.findOne({ email, value: verificationCodeValue });
    if (verificationCode) {
...
const deleteCode = async (verificationCode) => {
    await connectToDatabase();

    await VerificationCode.findByIdAndDelete(verificationCode._id);
}

export { generateCode, getCode, verifyCode, deleteCode };